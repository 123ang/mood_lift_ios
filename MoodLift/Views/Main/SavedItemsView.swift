    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            ...
            Image(systemName: "bookmark")
                .font(.system(size: 52))
                .foregroundStyle(.lightText.opacity(0.4))
                ...
                HStack {
                    ...
                    if let savedAt = item.savedAt {
                        Text(savedAt.formatted(date: .abbreviated, time: .omitted))
                            .font(.caption)
                            .foregroundStyle(.lightText.opacity(0.7))
                    }
                    ...
                    Button(action: onDelete) {
                        Image(systemName: "trash")
                            .font(.caption)
                            .foregroundStyle(.errorRed.opacity(0.7))
                            .padding(8)
        ...
        VStack(alignment: .leading, spacing: 4) {
            ForEach(options, id: \.0) { label, value in
                HStack(spacing: 6) {
                    Text("\(label).")
                        .font(.caption.weight(.bold))
                        .foregroundStyle(label == item.correctOption ? .successGreen : .lightText)
                    Text(value)
                        .font(.caption)
                        .foregroundStyle(label == item.correctOption ? .successGreen : .lightText)
